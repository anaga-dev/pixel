...
function getOffsetCoordinates(event, element) {
  ...
  return {
    x: Math.max(0, Math.min(width, event.clientX - left)),
    y: Math.max(0, Math.min(height, event.clientY - top))
  }
}
...
function updateColor(e) {
  ...
  if(e.pressure === 0) isDragging.value = false
...
function stopDraggingOutside(e) {
  stopDragging(e)
  const { x, y } = getOffsetCoordinates(e, canvas.value)
  lastCoords.x = x
  lastCoords.y = y
